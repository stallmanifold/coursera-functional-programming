...
object FunSets {
   ...
   */
    def forall(s: Set, p: Int => Boolean): Boolean = {
      def inBounds: Set = (a: Int) => (a >= -bound) && (a <= bound)
      def boundedInS: Set = filter(s, inBounds)

      def iter(a: Int): Boolean = {
        if (boundedInS(a) && !p(a)) false
   ...
   */
    def map(s: Set, f: Int => Int): Set = _ => s(f(_))
